...
$query = $pdo->query("SELECT * FROM responsaveis where cpf = '$responsavel' ");
	...
	$total_reg = @count($res);
	if($total_reg == 0){
		echo 'CPF do Responsável não encontrado ou não Cadastrado no sistema!';
		exit();
	}
}
...
//VERIFICAR SE O NÚMERO DO NIS JÁ EXISTE NO BANCO
if($antigo2 != $nis){
	$query = $pdo->query("SELECT * FROM alunos where nis = '$nis' ");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){
		echo 'O Nº de NIS já está Cadastrado!';
		exit();
	}
}
...
if($id == ""){
	$res = $pdo->prepare("INSERT INTO alunos SET nome = :nome, nis = :nis, cpf = :cpf, email = :email, endereco = :endereco, telefone = :telefone, foto = '$imagem', sexo = :sexo, data_nascimento = :data_nascimento, responsavel = :responsavel, data_cadastro = curDate()");	
	...
	if($imagem == "sem-foto.jpg"){
		$res = $pdo->prepare("UPDATE alunos SET nome = :nome, nis = :nis, cpf = :cpf, email = :email, endereco = :endereco, telefone = :telefone, sexo = :sexo, data_nascimento = :data_nascimento, responsavel = :responsavel WHERE id = '$id'");
	}else{
		$res = $pdo->prepare("UPDATE alunos SET nome = :nome, nis = :nis, cpf = :cpf, email = :email, endereco = :endereco, telefone = :telefone, foto = '$imagem', sexo = :sexo, data_nascimento = :data_nascimento, responsavel = :responsavel WHERE id = '$id'");
	}
